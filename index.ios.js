...
class AdvancedWebView extends React.Component {
  ...
  static propTypes = {
    ...
    /**
     * Function which notifies when an image has started to download
     */
  ...
  */
  ...
  _onImageDownloadComplete = (event: Event) => {
    if (!this.props.onImageDownloadComplete) {
      return;
    }
    this.props.onImageDownloadComplete(event.nativeEvent);
  };

  _onImageDownload = (event: Event) => {
    if (!this.props.onImageDownload) {
      return;
    }
    this.props.onImageDownload(event.nativeEvent);
  };

  _onLoadingError = (event: Event) => {
    /*if(event && event.nativeEvent && event.nativeEvent.domain =="WebKitErrorDomain" && event.nativeEvent.code == 101){
      return;
    }*/
